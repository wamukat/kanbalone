...
チケット、コメント、タグ、ブロッカー、親子関係、Resolved 状態、アーカイブをひとつの画面で扱えます。JSON API もあるので、人間が画面で見ながら、スクリプトやエージェントが同じデータを更新する運用にも向いています。
...
作成後は、Kanban / List の切り替え、検索、Resolved、タグ、Archived フィルタなど、通常の操作領域が表示されます。

![Kanban overview](../assets/user-guide/01-kanban-overview.png)

## まず触る場所

左側のサイドバーでボードを選び、中央でチケットを動かします。上部には検索、Resolved 状態、タグ、Archived のフィルタがあります。

- `Kanban` はレーンごとに流れを見る画面です。
- `List` は複数チケットをまとめて操作する画面です。
- タグはサイドバーにも並び、ボード内のテーマをすばやく見つけられます。

## ボードを作る

左上の `+` を押すと、ボード一覧の中にそのまま入力欄が出ます。名前を入力して Enter で作成できます。

Esc でキャンセルできます。空のままフォーカスを外してもキャンセルされます。
...
## アーカイブ済みチケットも必要な時だけ見る

通常は Archived チケットを隠して、今見るべき作業だけに集中できます。
...
3. 作業テーマをタグにする。
4. 詰まっている作業は Blockers に入れる。
5. 大きい作業は Parent / Children で分ける。
6. 週末やリリース前に List 表示でまとめて整理する。
7. 終わった作業は Resolved にし、もう見なくてよいものは Archive する。
