...
検索欄に入力がある時は、右端の `x` で検索条件をすぐに消せます。最後に開いていたボード、Kanban/List 表示、ボードごとのフィルタ条件、`Status` / `Priority` の開閉状態はブラウザに保存され、次回アクセス時に復元されます。
...
タグはサイドバーの Tags セクションにある `+` から、その場で名前だけ入力して作成できます。色は後からタグバッジをクリックして編集できます。

チケット編集中に新しいタグが必要になった場合は、Tags の入力欄に名前を入れて `+` を押すと、色なしタグとしてすぐ作成されます。色を付けたい時は、作業後にサイドバーのタグバッジから編集してください。
...
歯車を開くと現在のボード名が表示されます。ボード名の横にあるペンアイコンを押すと、その場でボード名を編集できます。
