...
Dark mode を実装する場合は、濃い青や紫に寄せず、墨色に近いニュートラルと teal を使います。
...
- Primary: Create, Save など確定操作。背景 `--accent`、文字 `#ffffff`。
- Secondary: 標準操作。背景 `--surface-1`、border `--border-soft`。
- Danger: Delete は foreground を `--danger`。削除確認の最終 Delete は背景 `--danger`、前景 `#ffffff`。
...
5. Dark mode は token 設計だけ先に用意し、実装は別作業として扱う。
