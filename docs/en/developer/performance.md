...
```bash
KANBALONE_PERF_OVERWRITE=true pnpm perf:seed
```

Seed a 5,000-ticket board:

```bash
KANBALONE_PERF_BOARD="Perf 5000" KANBALONE_PERF_TICKETS=5000 KANBALONE_PERF_OVERWRITE=true pnpm perf:seed
```
...
```bash
pnpm perf:benchmark
KANBALONE_PERF_BOARD="Perf 5000" pnpm perf:benchmark
```

## Notes

- `perf:seed` creates tags, tickets, comments, blockers, and one-level parent/child links through `POST /api/boards/import`.
- `perf:seed` will not delete an existing board with the same name unless `KANBALONE_PERF_OVERWRITE=true` is set.
